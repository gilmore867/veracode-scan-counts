...
def get_incomplete_scans(app_info):
    ...
    try:
        ...
        if static_scan != None and static_scan["status"] != 'PUBLISHED':
           log.info('The status of the static policy for application profile {} '.format(app_info.get('profile').get('name')) + 'with url ' + us_com + '{} was {}'.format(static_scan["scan_url"],static_scan["status"]))
           data = {incompletescancount: ['{} '.format(app_info.get('profile').get('name')), static_scan["status"], us_com + '{}'.format(static_scan["scan_url"])]}
